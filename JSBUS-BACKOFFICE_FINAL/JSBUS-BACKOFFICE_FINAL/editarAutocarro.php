					...
					<div class="col-xl-9 mx-auto">
						<div class="cta-inner text-center rounded">
							<h2 class="section-heading mb-5">
								<span class="section-heading-lower">Editar dados do Autocarro</span>
							</h2>
							...
							if(!isset($_SESSION['email'])or ($_SESSION["email"])!='admin@gmail.com'){
							...
							}else
								{
									...
									$sql = "SELECT * FROM autocarro WHERE id_autocarro=$id_autocarro";
									...
									{// vai buscar ha base de dados os dados nela guardada e poem os na tabela
										...
										echo'<form action="editarAF.php" method="GET">';
										echo'<input type="hidden" name="id_autocarro" value='.$row['id_autocarro'].' size="10"/><br>';
										echo'<label>Modelo </label><input type="text" name="modelo" value='.$row['modelo'].' size="10"/><br>';
										echo'<label>Matricula </label><input type="text" name="matricula" value='.$row['matricula'].' size="10"/><br>';
										echo'<label>Número</label><input type="text" name="numero" value='.$row['numero'].' size="10"/><br>';
										echo'<label>Lotação</label><input type="text" name="lotacao" value='.$row['lotacao'].' size="10"/><br>';
										echo'<label>Data de registo</label><input type="date" name="dataReg" value='.$row['dataReg'].' size="10"/><br>';
